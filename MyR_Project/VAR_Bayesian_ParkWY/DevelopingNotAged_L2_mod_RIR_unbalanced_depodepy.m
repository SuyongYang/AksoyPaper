...
% dates of the sample
y_all   = 1122;                 % number of all observations
y_vars  = 6;                  % number of variables
y_start = 1;                   % start date (obs number) (after y_start, first 'pen' obs are used as initial obs)
y_end   = 1122;                 % end date (obs number)

pen     = 2;                  % lags for endogenous variables

dates   = (1974:1:2020)'; % dates (1974 - 2020)
dates2  = dates((1+pen):end);    % dates excluding the training sample and the initial conditions

% data file
datanms = 'DevelopingNotAged_L2_mod_RIR_wholly_unbalanced_DEPO_DEPY.xls';

% folder and file names
folder  = 'm_01';
filenms = 'DevelopingNotAged_L2_mod_RIR_wholly_unbalanced_DEPO_DEPY_results';
...
%3; Real price of oil 
% varnms = {  'DEPO'; 'DEPY'; 'lnRgdp';'SAV';'CUR';'RIR'};   
% shocknms = {'DEPO'; 'DEPY';  'lnRgdp';'SAV';'CUR';'RIR'};
varnms = {  'DEPO'; 'DEPY'; 'RGDP';'SAV';'CUR';'RIR'};   
shocknms = {'DEPO'; 'DEPY';  'RGDP';'SAV';'CUR';'RIR'};
...
%dat = table2array(dat);
%rescale
%dat = table2array(dat).*[100 100 1 100 100 100 100 100 1 100 100 100 100 100 1 100 100 100  ones(95,1)'];
dat = table2array(dat).*[100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100  ones(95,1)'];

if (y_all~=size(dat,1))
    error('Check the nubmer of observations!')
end

if (y_vars~=size(dat(:,1:6),2))
...
irfslsumm.p5     = quantile(irfsl,0.025,4);
irfslsumm.p95    = quantile(irfsl,0.975,4);
...
%yx = [-1 1;-1 1;-2 2;-0.5 0.5;-0.5 0.5;-0.5 0.5];
yx = [-1 1;-1.5 1.5;-2 2;-0.5 0.5;-0.5 0.5;-0.5 0.5];
