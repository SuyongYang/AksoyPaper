...
% data file
%datanms = 'DATA_FINAL07 for PBVAR04_g7_L4_R_mod02_balanced_Pre.xls';
datanms = 'DATA_FINAL07 for PBVAR04_g7_L4_R_mod02_balanced_Pre_Con.xls';

datanms01 = 'DATA_FINAL07 for PBVAR04_g7_L4_R_mod02_balanced_RealPanel_Sys.xls';
% folder and file names
folder  = 'm_01';
filenms = 'DATA_FINAL07 for PBVAR_g7_L4_mod_RIR_balanced_results';
...
dat = datt(:,2:50);
%dat = table2array(dat);
dat = table2array(dat).*[100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 ones(24,1)'];
...
PreXX     = X'*X;
%iXX    = inv(XX);
PreBOLS   = PreXX\(X'*Y); % inv(X'*X)*(X'*Y)
BOLS = [PreBOLS;-sum(PreBOLS(21:26,:));-sum(PreBOLS(27:43,:))];
%UOLS   = Y - X*BOLS;
%S      = UOLS'*UOLS;
%OMGOLS = S/T;


datt01 = readtable(datanms01);
dat01 = datt01(:,2:52);
%dat = table2array(dat);
dat01 = table2array(dat01).*[100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 ones(26,1)'];
...
% 
%Y=Y-X(:,end-17:end)*BOLS(end-17:end,:)
XX     = X'*X;
%iXX    = pinv(XX);
%iXX    = inv(XX);
