...
findFirstAlpha <- function(equation)
{
...
# (e.g. p1,p2...)
getAlphas <- function(equations)
{
	firstAlphaIndex <- findFirstAlpha(equations[[1]])
	alphas<-attr(terms(equations[[1]]),"term.labels")[firstAlphaIndex:(firstAlphaIndex+6)]
}
...
#	removeOilPrice<-.~.-lag(log(Oil.Price))-lag(log(Oil.Price),2)

	for(name in names(equations))
	{
		runBothEffects(longNames[[name]],equations[[name]],getAlphas(equations),adjustTwoWays=removeOilPrice)
	}
}
...
extractVARMatrices <- function(model)
{
	...
	firstAlphaIndex<-findFirstAlpha(model[[1]]$formula)
...
#	matrices$A2 <- t(as.matrix(sapply(model,function(x){x$coefficients[8:14+offSet]})))
	matrices$B  <- t(as.matrix(sapply(model,function(x){x$coefficients[7:(firstAlphaIndex-1)+offSet]})))
	matrices$D  <- t(as.matrix(sapply(model,function(x){x$coefficients[firstAlphaIndex:(firstAlphaIndex+length(demoVars)-1)+offSet]})))
...
resultTables<-function(path="../tex/tables")
{
  ...
  print(fixRowNames(resTab2),sanitize.text.function=function(text){text}, file=paste(path,"/resultsHRPi.tex",sep=""))
  print("Done")
}
...
demoRangesByCountry<-function()
{
  ...
  rownames(result)<-result[,1]
  result[,1]<-NULL
  colnames(result)<-c("G","I","S","H","rr","$\\pi$")
...
plotDemoMeansLines<-function()
{
  ...
  par(new=F)
#  dev.copy2pdf(file=paste('../tex/plotDemoMeans.pdf',sep=''))
}


#{
#  plot(plotDemoMeansLines)
#  dev.copy2pdf(file=paste('../tex/plotDemoMeans.pdf',sep=''))
#}
