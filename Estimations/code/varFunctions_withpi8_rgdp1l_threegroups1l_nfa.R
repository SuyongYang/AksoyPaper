...
extractVARMatrices <- function(model)
{
	...
	matrices$A1 <- t(as.matrix(sapply(model,function(x){x$coefficients[1:8+offSet]})))
#	matrices$A2 <- t(as.matrix(sapply(model,function(x){x$coefficients[8:14+offSet]})))
	matrices$B  <- t(as.matrix(sapply(model,function(x){x$coefficients[9:(firstBetaIndex-1)+offSet]})))
...
resultTables<-function(path="../tex/tables")
{
  ...
  resTab2<-multiResultTable(varModel$h,varModel$rr,varModel$rad1, varModel$nfa_gdp,varModel$pi)
  caption(resTab2)<-"Results for Hours, Interest Rate, R\\&D, NFA and Inflation"
...
demoRangesByCountry<-function()
{
  ...
  rownames(result)<-result[,1]
  result[,1]<-NULL
  colnames(result)<-c("y","I","S","H","rr","rad1","$\\nfa_gdp$","$\\pi$")
