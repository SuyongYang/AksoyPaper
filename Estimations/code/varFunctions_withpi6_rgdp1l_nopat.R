...
#	removeOilPrice<-.~.-lag(log(Oil.Price))-lag(log(Oil.Price),2)-popGrowth-lag(popGrowth,1)-Birth.Rate-Death.Rate
...
resultTables<-function(path="../tex/tables")
{
  ...
  caption(resTab2)<-"Results for Hours, Interest Rate, R\\&D and Inflation"
...
demoRangesByCountry<-function()
{
  ...
  rownames(result)<-result[,1]
  result[,1]<-NULL
  colnames(result)<-c("G","I","S","H","rr","$\\RD$")
